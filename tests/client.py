...
if __name__ == '__main__':
    ...
    try:
        ...
        for i in range(args.iterations):
            ...
            ping_output = ping_runner.run_ping(args.server_ip, extra_args=["-c", "4"])
            rtt_ms = ping_runner.parse_rtt_avg(ping_output) if ping_output else None
            packet_loss_ping = ping_runner.parse_packet_loss(ping_output) if ping_output else None

            database.insert_iteration(conn, test_id, i + 1, throughput, jitter_ms, packet_loss, packet_loss_ping, rtt_ms, duration)
        ...
        #print(f"  Loss:       {avg['packet_loss_iperf']:.2f}%")
        #print(f"  Ping Loss:  {avg['packet_loss_ping']:.2f}%")
